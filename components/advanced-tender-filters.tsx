...
const SEARCH_FIELD_OPTIONS: {
  ...
  },
  {
    value: "title",
    label: "Title",
    description: "Search in tender titles only",
  },
  {
    value: "description",
    label: "Description",
    description: "Search in descriptions and bid details",
  },
  {
    value: "department",
    label: "Department",
    description: "Search in department names",
  },
  {
    value: "procuringEntity",
    label: "Procuring Entity",
    description: "Search in organization names",
  },
  {
    value: "location",
    label: "Location",
    description: "Search in delivery locations",
  },
  {
    value: "contactPerson",
    label: "Contact Person",
    description: "Search in contact information",
  },
  {
    value: "specialConditions",
    label: "Special Conditions",
    description: "Search in special requirements",
  },
];
...
export default function AdvancedTenderFilters({
  ...
  const [savedSearches, setSavedSearches] = useState<
    Array<{ name: string; query: string; fields: SearchField[] }>
  >([]);
            ...
            <div key={option} className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked: boolean) =>
                  handleMultiSelectChange(key, option, checked)
                }
              />
              <Label
                htmlFor={`${key}-${option}`}
  ...
  // Render date filter
  const renderDateFilter = (title: string, key: keyof TenderFilters) => {
                  ...
                  <Popover key={option.value}>
                    <PopoverTrigger asChild>
                      <Badge
                        ...
                        onClick={() => {
                          const newFields = filters.searchFields.includes(
                            option.value
                          )
                            ? filters.searchFields.filter(
                                (f) => f !== option.value
                              )
                            : [
                                ...filters.searchFields.filter(
                          ...
                          if (option.value === "all") {
                            ...
                            handleSearchFieldsChange(
                              newFields.length > 0 ? newFields : ["all"]
                            );
                          }
                        }}
                      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="space-y-4">
                ...
                {renderMultiSelectFilter(
                  "Categories",
                  "categories",
                  filteredResult.filterOptions.categories
                )}
                {renderMultiSelectFilter(
                  "Departments",
                  "departments",
                  filteredResult.filterOptions.departments
                )}
                {renderMultiSelectFilter(
                  "Status",
                  "statuses",
                  filteredResult.filterOptions.statuses
                )}
                {renderMultiSelectFilter(
                  "Locations",
                  "locations",
                  filteredResult.filterOptions.locations
                )}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      {filteredResult.filterOptions.currencies.map(
                        (currency) => (
                ...
                {renderBooleanFilter(
                  ...
                  "hasBriefing",
                  "Tenders that have briefing sessions"
                )}
                {renderBooleanFilter(
                  "Compulsory Briefing",
                  "isCompulsoryBriefing",
                  "Briefing sessions that are mandatory"
                )}
                {renderBooleanFilter(
                  "Has Special Conditions",
                  "hasSpecialConditions",
                  "Tenders with special conditions"
                )}
                {renderBooleanFilter(
                  "Has Contact Info",
                  "hasContactInfo",
                  "Tenders with contact information"
                )}
                {renderBooleanFilter(
                  "Has Email",
                  "hasEmail",
                  "Tenders with email contact"
                )}
                {renderBooleanFilter(
                  "Has Phone",
                  "hasPhone",
                  "Tenders with phone contact"
                )}
              </div>
            </div>
          </div>
